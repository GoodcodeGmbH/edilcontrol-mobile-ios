...
class LoginViewModel: ObservableObject {
    ...
    func getAuthSession() async {
        ...
        guard let url = URL(string: "\(Constant.authUrl)\(Constant.sessionEndpoint)") else {
            print("Invalid URL")
    ...
    func requestOTP(username: String, password: String, authSession: AuthSession, otp: String, applicationId: String) async {
        ...
        guard let url = URL(string: "\(Constant.authUrl)\(Constant.otpRequestEndpoint)") else {
            print("Invalid URL")
    ...
    func validateOTP(username: String, password: String, authSession: AuthSession, otp: String, applicationId: String, completion: @escaping () async -> Void) async {
        ...
        var request = URLRequest(url: url)
        request.httpMethod = Constant.post
        request.addValue(Constant.applicationJson, forHTTPHeaderField: Constant.contentType)
    ...
    func getTenantList(completion: @escaping () -> Void) async {
        ...
        let url = URL(string: "\(Constant.tenantUrl)\(Constant.selfEndpoint)")!
        
        var request = URLRequest(url: url)
        request.httpMethod = Constant.get
        request.addValue("\(Constant.bearer) \(token)", forHTTPHeaderField: Constant.authorization)
